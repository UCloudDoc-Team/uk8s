...
相较于普通Pod，使用Udisk的Pod调度涉及到了UDisk自身挂载规则的限制，更为复杂。其中UDisk的限制为

- 普通云盘和SSD磁盘挂载要求必须与云主机处于相同可用区
- RSSD云盘挂载要求与云主机处于相同RDMA区（RDMA区小于可用区）
- RSSD云盘仅可以挂载到快杰云主机
...
- 当Pod遇到重建等重新调度的过程中，如何保证新调度的节点满足云盘挂载的要求

UK8S提供的csi-udisk插件，依赖K8S提供的插件能力，帮助用户实现了尽可能少的介入，以下以SSD udisk为例进行讲解。
### 9.1 创建PV时自动创建UDisk
从上面的文档中可以了解到，当pvc创建完成时，csi会自动创建pv以及udisk，并完成绑定工作。但是创建哪个可用区的UDisk呢，如果随意选择，则会导致后续Pod调度完成后无法挂载云盘。

为此K8S提供了WaitForFirstConsumer机制。当`StorageClass`中指定了`volumeBindingMode: WaitForFirstConsumer`参数时，CSI不会立刻创建PV及云盘，以下为`WaitForFirstConsumer`模式下的工作流程。
...
- CSI更新PV中的`spec.nodeAffinity`字段，记录云盘所在的可用区信息

按照以上逻辑，可以保证Pod调度后创建的云盘可以顺利挂载的相应主机
...
- 如果所有节点都不满足磁盘调度要求，会上报`had volume node affinity conflict`类型的EVENT
- K8S调度器在上一步过滤的基础上，在可调度的节点上，继续按照普通Pod调度流程进行调度