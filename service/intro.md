...
ULB 提供了`应用型负载均衡 ALB`、`网络型负载均衡 NLB`、`传统型负载均衡 CLB`（又分为报文转发型和请求代理型）等多种负载均衡类型。建议您优先考虑使用`ALB`或者`NLB`。

如果你希望对 ULB 有深入的了解，请访问[ULB 产品介绍](ulb/README)，

在 UK8S 中，我们支持将 Service 的入口配置为 ALB、NLB 或 CLB。您可以通过 Service yaml 中 [Annotations](/uk8s/service/annotations) 的形式自行配置各项参数 。
