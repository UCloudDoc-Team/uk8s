...
### 七、ALB/ULB7 类型 Ingress 获取客户端源 IP

当 Ingress Controller 前端有七层负载均衡（ALB/ULB7）时，Ingress Controller 获取到的源 IP 是负载均衡的出口 IP，而非真实的客户端 IP。为了让 Ingress Controller 能获取到客户端的真实 IP，需要修改 `ingress-nginx-controller` 的 ConfigMap。
...
此外，还需要配置 `proxy-real-ip-cidr`，用于指定允许信任的负载均衡器出口 IP 地址段。只有来自这些 IP 的 `X-Forwarded-For` 头才会被信任，防止伪造。
...
2.  在 `data` 字段中添加或修改以下配置：
  ...
  *   `use-forwarded-headers`: 启用 `X-Forwarded-*` 系列请求头。
  *   `forwarded-for-header`: 设置获取客户端真实 IP 的请求头，这里使用 `X-Forwarded-For`。
  *   `proxy-real-ip-cidr`: 允许信任的负载均衡器出口 IP 段，多个网段用逗号分隔。只有来自这些 IP 的 `X-Forwarded-For` 头才会被信任，防止伪造。

配置完成后，Ingress Controller 即可正确获取到客户端真实 IP。
