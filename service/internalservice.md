...
传统型负载均衡 CLB 为历史存量产品，其本质是共享型负载均衡实例，无性能保证。CLB 有[一系列配额限制](/ulb/intro/limit)，且将逐步缩紧使用权限。

因此如有七层代理的需求，推荐您使用[应用型负载均衡ALB](/ulb/alb/intro/whatisalb)；如有四层代理的需求，推荐您使用[网络负载均衡NLB](/ulb/NLB/intro/whatisnlb)。

> ⚠️ 如需使用 ALB 或 NLB 产品，请升级 cloudprovider 版本到 `24.12.24` 及以上，参考[CloudProvider 插件更新](/uk8s/service/cp_update)。

* svc关联ULB删除

如 ULB 为 UK8S 在创建 Service 时同步创建，删除 Service 时 ULB 会同步删除，请勿在 ULB 上手动创建 Vserver，如需多个 Service 共用 ULB，可先创建 ULB，并在创建 Service 时关联已有 ULB，详情请见[使用已有 ULB](/uk8s/service/ulb_designation)。
...
metadata:
  ...
  annotations:
    # ULB访问模式：outer为外网，inner为内网；outer为默认值，此处可省略；
...
如果您需要通过CLB7暴露HTTP/HTTPS服务，请参考 [ULB参数说明](/uk8s/service/annotations) 进行配置。

## 3. 通过NLB或CLB4对外暴露TCP服务

 使用 NLB 时，推荐升级 [CloudProvider](/uk8s/service/cp_update) 版本到 >= 24.12.24。

对于TCP协议的服务，使用CLB4时，如果只在内网暴露，只需要在metadata.annotations 指定 `service.beta.kubernetes.io/ucloud-load-balancer-type` 为 "inner"，如果外网暴露则设置为 "outer"，其他参数都有默认值，可不填写。

使用网络型负载均衡NLB时，可以增加一条annotation: `"service.beta.kubernetes.io/ucloud-load-balancer-listentype": "network"`。
...
metadata:
  ...
  annotations:
    ...
    "service.beta.kubernetes.io/ucloud-load-balancer-listentype": "network"
     # 用于声明VServer协议类型，并非应用协议
    "service.beta.kubernetes.io/ucloud-load-balancer-vserver-protocol": "tcp"
     # 不论容器端口类型是tcp还是udp，均建议显式声明为port。
...
## 4. 通过NLB或CLB4对外暴露UDP服务

如果您的应用是UDP协议，则务必显式声明健康检查的类型为port（端口检查），否则默认为ping，可能导致ULB误认为后端业务不正常。如果需要外网暴露请注意修改 ucloud-load-balancer-type 为 "outer"。
...
目前针对UDP协议的健康检查支持ping和port两种模式，默认为ping，强烈推荐改为port；
...
**需要注意的是UDP回包长度不要超过1440，以避免可能的分片导致NLB/CLB4无法收到健康检查响应，导致健康检查失败。**


## 5. 通过NLB或CLB4对外暴露服务（TCP和UDP协议混用）

以下示例中，svc对外暴露了两个端口，其中80端口使用TCP协议，53端口使用UDP协议。
...
metadata:
  ...
  annotations:
    ...
    # 选择ULB类型，“network”表示使用网络型负载均衡NLB
    "service.beta.kubernetes.io/ucloud-load-balancer-listentype": "network"
    # VServer协议类型，使用tcp/udp或省略时时，同时支持TCP和UDP协议
...
## 6. LB后端直接使用Endpoint（direct-endpoint）

> ⚠️ 使用 direct-endpoint 时，需要升级 [CloudProvider](/uk8s/service/cp_update) 版本到 >= 25.07.23。
>
> ⚠️ 仅在 ALB/NLB 下支持direct endpoint，且在这种情况下Service extenalTrafficPolicy [流量策略](/uk8s/service/svc_trafficpolicy)失效
