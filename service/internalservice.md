...
**3. 内网ULB已支持UDP协议，目前灰度中，如需使用，请联系UCloud技术支持。**

>> 备注: 目前ULB4针对UDP协议的健康检查支持ping和port两种模式，默认为ping，强烈推荐改为port，port健康检查是对UDP端口发送UDP报文( "Health Check" 字符串)和针对RS IP发送ICMP Ping报文。 如果超时时间内回复了UDP报文则认为健康；如果超时时间内回复了ICMP端口不可达报文，则认为不健康；如果超时时间没收到UDP回包，则以Ping的探测结果为准。因此您的应用程序需要响应UDP健康检查报文，**需要注意的是UDP回包长度不要超过1440，以避免可能的分片导致ULB4无法收到健康检查响应，导致健康检查失败。**
...
如果你的应用是UDP协议，则需要显示声明健康检查的类型为port(端口检查)，而非ping。具体如下：
