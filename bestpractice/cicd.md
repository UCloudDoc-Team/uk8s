...
{{:compute:uk8s:bestpractice:jenkins-arch.png?600|}}
...
<code>kubectl apply -f http://uk8s.cn-bj.ufileos.com/yaml%2Fjenkins%2Fjenkins-pvc.yaml</code>

3、以Deployment方式部署Jenkins master,为了演示方便，我们还使用LoadBalancer类型的service将其暴露到外网。

<code>kubectl apply -f http://uk8s.cn-bj.ufileos.com/yaml%2Fjenkins%2Fjenkins.yaml</code>
...
<code>kubectl apply -f http://uk8s.cn-bj.ufileos.com/yaml%2Fjenkins%2Fjenkins-rbac.yaml</code>

### 三、安装Kubernetes插件

1、前面我们已经获取到Jenkins的外网IP地址，我们直接在浏览器输入EXTERNAL-IP:8080，即可打开Jenkins页面，提示需要输入初始化密码：

{{:compute:uk8s:bestpractice:unlock.png?600|}}

2、我们通过kubectl log获取jenkins容器的日志来获取初始化密码

<code>kubectl logs jenkins-deployment-66b865dbd-xvmdz -n jenkins
</code>

{{:compute:uk8s:bestpractice:passget.png?600|}}
...
{{:compute:uk8s:bestpractice:installplugin.png?600|}}
...
{{:compute:uk8s:bestpractice:certificate.png?600|}}
...
{{:compute:uk8s:bestpractice:pinju.png?600|}}

{{:compute:uk8s:bestpractice:kubeconfig.jpeg?600|}}
...
2.1 点击”new item“，输入Job名称，选择Pipeline，点击"OK"。
...
{{:compute:uk8s:bestpractice:1.png?600|}}
...
{{:compute:uk8s:bestpractice:2.png?600|}}
